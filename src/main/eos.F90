...
module eos
 ...
 !--Define initial parameters for Helmholtz EOS
 !
...
!  SOMETHING GOT TO CHANGE AS FOR HELMHOLTZ TEMPERATURE IS NEEDED TOO
...
subroutine equationofstate(eos_type,ponrhoi,spsoundi,rhoi,xi,yi,zi, &
 ...
 real,    intent(in), optional :: tempi  !CHNGCODE ! QUIZAS PODEMOS METER LA TEMPERATURA DE LA MISMA FORMA
...
!
 ...
 !   Need Temperature[K] Density[g/cm**3] and composition
    cgsrhoi = rhoi * unit_density
    abar = 
    zbar = 
    
    call get_eos_pressure_soundspeed_helmholtz(tempi,cgsrhoi,abar,zbar,cgsptot,cgsspsound)
    
    ptot = cgsptot/ unit_pressure
    ponrhoi= ptot / rhoi
