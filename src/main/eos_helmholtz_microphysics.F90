...
!____________NUEVAS DECLARACIONES_________________________________________________

! Size of the helmholtz free energy tables 
 integer, parameter :: imax = 271, jmax = 71

! for the electrons density and temperature
...
!____________AQUI ACABAN LAS NUEVAS DECLARACIONES_________________________________________________________
...
!_______________________________________________________________________
 subroutine read_eos_helmholtz(ierr)
...
! declare local variables
      ...
      fnum = 120
      ierr = 0 ! HAY QUE AÑADIR UN ERROR MECHANISM POR SI NO ENCONTRAMOS EL FILE
      filename = 'helm_table.dat'
      
! open the file (use softlinks to input the desired table)
      filename = find_phantom_datafile(filename,'eos/helmholtz')
      inquire(file=trim(filename), exists=exists)
      if (exists) then

        open(unit=fnum,file=filename,status='old')
...
! read the helmholtz free energy and its derivatives
        do j=1,jmax
         ...
         do i=1,imax
          dsav = dlo + (i-1)*dstp
          d(i) = 10.0d0**(dsav)
          read(fnum,*) f(i,j),fd(i,j),ft(i,j),fdd(i,j),ftt(i,j),fdt(i,j), &
...
! read the pressure derivative with density table
        do j=1,jmax
         do i=1,imax
          read(fnum,*) dpdf(i,j),dpdfd(i,j),dpdft(i,j),dpdfdt(i,j)
         enddo
        enddo
...
! read the electron chemical potential table
        do j=1,jmax
         do i=1,imax
          read(fnum,*) ef(i,j),efd(i,j),eft(i,j),efdt(i,j)
         enddo
        enddo
...
! read the number density table
        do j=1,jmax
         do i=1,imax
          read(fnum,*) xf(i,j),xfd(i,j),xft(i,j),xfdt(i,j)
         enddo
        enddo
...
!      write(6,*)
  ...
  return
 end subroutine read_eos_helmholtz
!_________READ HELMHOLTZ TABLE__________________________________________
!_______________________________________________________________________



 subroutine helmeos(temp, den, abar, zbar, pres, sound, ierr)

!      include 'const.dek' REMEMBER TO INCLUDE CONSTANTS
...
end module helmholtz_microphysics
